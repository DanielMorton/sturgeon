...
fn make_grid<T>(rows: usize, cols: usize) -> Result<Vec<Vec<T>>, Box<dyn Error>>
where
    T: Num + Copy,
{
...
mod tests {
    ...
    fn test_longest_path2() -> Result<(), Box<dyn Error>> {
        let down = vec![vec![20, 0, 0], vec![20, 0, 0]];
        let right = vec![vec![0, 0], vec![0, 0], vec![10, 10]];
    ...
    fn test_longest_path3() -> Result<(), Box<dyn Error>> {
        let down = vec![vec![0, 0, 20], vec![0, 0, 20]];
        let right = vec![vec![10, 10], vec![0, 0], vec![0, 0]];
    ...
    fn test_longest_path4() -> Result<(), Box<dyn Error>> {
        let down = vec![vec![20, 0, 0], vec![0, 0, 0]];
        let right = vec![vec![0, 30], vec![0, 0], vec![0, 0]];
